...
import spwrap.result.ListResult;
...
public class Executor {
	 ...
	 * @param rsMapper
	 * @return
	 * @see {@link #execute(String, List, List, OutputParamMapper, ResultSetMapper)}
	 */
	public final <T> ListResult<T> execute(String procName, List<Param> inParams, ResultSetMapper<T> rsMapper) {
		return execute(procName, inParams, null, null, rsMapper).toListResult();
	}
	 ...
	 * @see {@link #execute(String, List, List, OutputParamMapper, ResultSetMapper)}
	 */
	public final Result execute(String procName, List<Param> inParams) {
		return execute(procName, inParams, null, null).toResult();
	}
	 ...
	 * @see {@link #execute(String, List, List, OutputParamMapper, ResultSetMapper)}
	 */
	public final Result execute(String procName) {
		return execute(procName, null, null, null).toResult();
	}
	 ...
	 * @see {@link #execute(String, List, List, OutputParamMapper, ResultSetMapper)}
	 */
	public final <T> ObjectResult<T> execute(String procName, List<ParamType> outParamsTypes,
			OutputParamMapper<T> paramMapper) {
		return execute(procName, null, outParamsTypes, paramMapper);
	}
	 ...
	 * @see {@link #execute(String, List, List, OutputParamMapper, ResultSetMapper)}
	 */
	public final <T> ObjectResult<T> execute(String procName, List<Param> inParams, List<ParamType> outParamsTypes,
			OutputParamMapper<T> paramMapper) {
		return execute(procName, inParams, outParamsTypes, paramMapper, null).toObjectResult();
	}
	 ...
	 * @return
	 */
	public final <T, U> ListObjectResult<T, U> execute(String procName, List<Param> inParams,
			List<ParamType> outParamsTypes, OutputParamMapper<U> paramMapper, ResultSetMapper<T> rsMapper) {
		...
		ListObjectResult<T, U> result = null;
		try {
			...
			final boolean success = call.getBoolean(resultCodeIndex);
			final String message = call.getString(resultCodeIndex + 1);

			result = new ListObjectResult<T, U>(success, message, list, object);

		} catch (Exception ex) {
			log.error("[" + callableStmt + "]", ex);
			result = new ListObjectResult<T, U>(false, ex.getMessage(), null, null);
		} finally {
